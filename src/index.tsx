...
export const InputMask: FunctionComponent<inputMaskPropsType> = ({ mask, type, onChange, ...rest }) => {
  ...
  function handler(event: eventChangeType) {
    if (type === "number") {
    ...
    event.target.value = event.target.value.slice(0, mask.length);
  ...
  return <input type="text" {...rest} value={inputMask} onChange={handler} />;
};
